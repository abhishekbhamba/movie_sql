...
SELECT actors.act_fname, actors.act_lname, movie_cast.role, movie.mov_title FROM actors
...
-- Q3 Write a SQL query to find who directed a movie that casted a role as ‘Sean Maguire’. 
-- Return director first name, last name and movie title.
...
-- Q4 Write a SQL query to find the actors who have not acted in any movie between1990 and 2000 (Begin and end values are included.). 
-- Return actor first name, last name, movie title and release year. 

SELECT actors.act_fname, actors.act_lname, movie.mov_title, movie.mov_year FROM actors
INNER JOIN movie_cast ON movie_cast.act_id = actors.act_id
INNER JOIN movie ON movie.mov_id = movie_cast.mov_id
WHERE movie.mov_year BETWEEN 1920 AND 2000;


-- Q5 Write a SQL query to find the directors with number of genres movies.
...
SELECT director.dir_fname, director.dir_lname, genres.gen_id, COUNT(genres.gen_title) AS "No. of genres movie" FROM director
INNER JOIN movie_direction ON movie_direction.dir_id = director.dir_id
...
ORDER BY director.dir_fname ASC;
...
SELECT movie.mov_title, movie.mov_year, genres.gen_title, director.dir_fname, director.dir_lname FROM director
...
SELECT director.dir_fname, director.dir_lname, movie.mov_title, movie.mov_time, movie.mov_year, movie.mov_dt_rel FROM director
INNER JOIN movie_direction ON movie_direction.dir_id = director.dir_id
INNER JOIN movie ON movie.mov_id = movie_direction.mov_id
